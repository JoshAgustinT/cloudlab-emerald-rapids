...
create_extfs() {
  record_log "Creating ext4 filesystem on /dev/nvme1n1"
...
prepare_local_partition() {
  ...
  MOUNT_POINT=$(mount -v | grep "/dev/nvme1n1" | awk '{print $3}' ||:)

  if [[ x"${MOUNT_POINT}" == x"${MOUNT_DIR}" ]]; then
    record_log "/dev/nvme1n1 is already mounted on ${MOUNT_POINT}"
    return
  fi

  if [ x$(sudo file -sL /dev/nvme1n1 | grep -o ext4) == x"" ]; then
    create_extfs
  fi
